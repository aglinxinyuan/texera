...
async def on_create_session(
    ws: WebSocket, sm: SessionManager, msg: CreateSessionRequest
) -> None:
    sid = await sm.new_session()
    await send_outgoing(
        ws, CreateSessionResponse(type="CreateSessionResponse", sessionId=sid)
    )


@handles(HeartBeatRequest)
async def on_heartbeat(
    ws: WebSocket, _sm: SessionManager, msg: HeartBeatRequest
) -> None:
    await send_outgoing(ws, HeartBeatResponse(type="HeartBeatResponse"))


@handles(OperatorSchemaResponse)
async def on_operator_schema_response(
    ws: WebSocket, sm: SessionManager, msg: OperatorSchemaResponse
) -> None:
    sid = sm.current_sid
    if not sid:
        return
...
async def on_add_operator_response(
    ws: WebSocket, sm: SessionManager, msg: AddOperatorAndLinksResponse
) -> None:
    sid = sm.current_sid
    if not sid:
        return
...
async def on_chat_user_message(
    ws: WebSocket, sm: SessionManager, msg: ChatUserMessageRequest
) -> None:
    ...
    if not text:
        await send_outgoing(
            ws, ErrorResponse(type="Error", error="Empty chat message.")
        )
        return
        ...
        try:
            async for delta in session.stream_chat(ws, text):
                await send_outgoing(
                    ws,
                    ChatStreamResponseEvent(
                        type="ChatStreamResponseEvent", delta=delta
                    ),
                )
            ...
            await send_outgoing(
                ws, ErrorResponse(type="Error", error="Streaming failed")
            )
        finally:
            await send_outgoing(
                ws, ChatStreamResponseComplete(type="ChatStreamResponseComplete")
            )
